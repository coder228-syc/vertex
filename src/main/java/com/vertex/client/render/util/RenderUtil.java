...
import static com.vertex.client.util.MathUtil.interpolate;
...
public class RenderUtil implements IMinecraft {
    ...
    private static final Supplier<SimpleFramebuffer> TEMP_FBO_SUPPLIER_2 = Suppliers.memoize(() -> new SimpleFramebuffer(mc.getWindow().getFramebufferWidth(), mc.getWindow().getFramebufferHeight(), false));
    ...
    private static void setShaderUniforms(ShaderProgram shader, float width, float height, Vector4f radius, float smoothness) {
        setUniformSafe(shader, "Size", width, height);
        setUniformSafe(shader, "Radius", radius.x, radius.y, radius.z, radius.w);
        setUniformSafe(shader, "Smoothness", smoothness);
    }

    private static void setUniformSafe(ShaderProgram shader, String name, float... values) {
    ...
    public static void drawRoundedRect(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, int color) {
        drawRoundedRect(matrices, x, y, width, height, rounding, 1.0f, color);
    }

    public static void drawRoundedRect(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float smoothness, int color) {
        enableRender();
        ShaderProgram shader = RenderSystem.setShader(ResourceProvider.RECTANGLE_SHADER_KEY);
        setShaderUniforms(shader, width, height, rounding, smoothness);
    ...
    public static void drawBlurredRoundedRectangle(MatrixStack matrices, float x, float y, float width, float height, float radius, int color, float alpha) {
        drawBlurredRoundedRectangle(matrices, x, y, width, height, new Vector4f(radius, radius, radius, radius), color, alpha);
    }

    public static void drawBlurredRoundedRectangle(MatrixStack matrices, float x, float y, float width, float height, Vector4f radius, int color, float alpha) {
        enableRender();
        ShaderProgram shader = RenderSystem.setShader(ResourceProvider.BLURRED_ROUND_RECT_SHADER_KEY);
        Framebuffer mainFbo = getMainFbo();
        RenderSystem.setShaderTexture(0, KawaseBlur.blur.getBlurredTexture());
        setUniformSafe(shader, "Resolution", (float) mainFbo.textureWidth, (float) mainFbo.textureHeight);
        shader.getUniform("Size").set(width, height);
        ...
        shader.getUniform("Color").set(rgba[0], rgba[1], rgba[2], rgba[3]);
        ShaderManager.vertexShader(matrices, x, y, width, height, -1);
    ...
    public static void drawTwoPassBlur(MatrixStack matrices, float x, float y, float width, float height, float rounding, float blurRadius, int color) {
        drawTwoPassBlur(matrices, x, y, width, height, new Vector4f(rounding, rounding, rounding, rounding), blurRadius, color);
    }

    public static void drawTwoPassBlur(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float blurRadius, int color) {
        final SimpleFramebuffer fbo1 = TEMP_FBO_SUPPLIER.get();
        final SimpleFramebuffer fbo2 = TEMP_FBO_SUPPLIER_2.get();
        final Framebuffer mainFbo = getMainFbo();

        if (fbo1.textureWidth != mainFbo.textureWidth || fbo1.textureHeight != mainFbo.textureHeight) {
            fbo1.resize(mainFbo.textureWidth, mainFbo.textureHeight);
        }
        if (fbo2.textureWidth != mainFbo.textureWidth || fbo2.textureHeight != mainFbo.textureHeight) {
            fbo2.resize(mainFbo.textureWidth, mainFbo.textureHeight);
        }

        enableRender();

        fbo1.beginWrite(false);
        mainFbo.draw(fbo1.textureWidth, fbo1.textureHeight);

        fbo2.beginWrite(true);
        ShaderProgram hShader = RenderSystem.setShader(ResourceProvider.HBLUR_SHADER_KEY);
        RenderSystem.setShaderTexture(0, fbo1.getColorAttachment());
        setUniformSafe(hShader, "Size", (float)fbo1.textureWidth, (float)fbo1.textureHeight);
        setUniformSafe(hShader, "Radius", 0f, 0f, 0f, 0f);
        setUniformSafe(hShader, "Smoothness", 1.0f);
        setUniformSafe(hShader, "BlurRadius", blurRadius);
        ShaderManager.vertexShader(matrices, 0, 0, mc.getWindow().getScaledWidth(), mc.getWindow().getScaledHeight(), -1);

        mainFbo.beginWrite(false);
        ShaderProgram vShader = RenderSystem.setShader(ResourceProvider.VBLUR_SHADER_KEY);
        RenderSystem.setShaderTexture(0, fbo2.getColorAttachment());
        setUniformSafe(vShader, "Size", (float)fbo2.textureWidth, (float)fbo2.textureHeight);
        setUniformSafe(vShader, "Radius", rounding.x, rounding.y, rounding.z, rounding.w);
        setUniformSafe(vShader, "Smoothness", 1.0f);
        setUniformSafe(vShader, "BlurRadius", blurRadius);
        ShaderManager.vertexShader(matrices, x, y, width, height, color);

        RenderSystem.setShaderTexture(0, 0);
        disableRender();
    }
    public static void drawMetanoise(MatrixStack matrices, float x, float y, float width, float height, float time, float radius, int bgColor, int outlineColor) {
        enableRender();
        ShaderProgram shader = RenderSystem.setShader(ResourceProvider.METANOISE_SHADER_KEY);
        shader.getUniform("Size").set(width, height);
        shader.getUniform("Time").set(time);
        shader.getUniform("BgColor").set(ColorUtil.getRed(bgColor) / 255f, ColorUtil.getGreen(bgColor) / 255f, ColorUtil.getBlue(bgColor) / 255f, ColorUtil.getAlpha(bgColor) / 255f);
        shader.getUniform("OutlineColor").set(ColorUtil.getRed(outlineColor) / 255f, ColorUtil.getGreen(outlineColor) / 255f, ColorUtil.getBlue(outlineColor) / 255f, ColorUtil.getAlpha(outlineColor) / 255f);
    ...
    public static void drawZoomBlur(MatrixStack matrices, float x, float y, float width, float height, float rounding, float strength, int color) {
        drawZoomBlur(matrices, x, y, width, height, new Vector4f(rounding, rounding, rounding, rounding), strength, color);
    }

    public static void drawZoomBlur(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float strength, int color) {
        final SimpleFramebuffer fbo = TEMP_FBO_SUPPLIER.get();
        final Framebuffer mainFbo = getMainFbo();
        ...
        ShaderProgram shader = RenderSystem.setShader(ResourceProvider.ZOOMBLUR_SHADER_KEY);
        RenderSystem.setShaderTexture(0, fbo.getColorAttachment());
    ...
    public static void drawLiquidRect(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float cornerSmoothness, float fresnelPower, float fresnelAlpha, float baseAlpha, boolean fresnelInvert, float fresnelMix, float distortStrength, ColorRGBA color) {
        drawLiquidRect(matrices, x, y, width, height, rounding, cornerSmoothness, fresnelPower, fresnelAlpha, baseAlpha, fresnelInvert, fresnelMix, distortStrength, color, false);
    }

    public static void drawLiquidRect(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float cornerSmoothness, float fresnelPower, float fresnelAlpha, float baseAlpha, boolean fresnelInvert, float fresnelMix, float distortStrength, ColorRGBA color, boolean useBlurTexture) {
        drawRoundedRect(matrices, x, y, width, height, rounding.x, (color.getAlpha() << 24) | (color.getRed() << 16) | (color.getGreen() << 8) | color.getBlue());
    }
    ...
    public static void drawTexture(MatrixStack matrices, Object texture, float x, float y, float width, float height, float rounding, int color) {
        ...
        if (texture instanceof String path) {
            textureId = Identifier.of("vertexclient", path);
        ...
        render3D.endBuilding(buffer);
        disableRender();
    }
    public static void drawCircleBorder2(MatrixStack matrices, float centerX, float centerY, float diameter, float thickness, int color) {
        float half = diameter / 2f;
        drawRoundedBorder(matrices, centerX - half, centerY - half, diameter, diameter, circleRounding2(diameter), thickness, color);
    }

    private static float circleRounding2(float diameter) {
    ...
    public static void drawCircleBorder(MatrixStack matrices, float centerX, float centerY, float diameter, float thickness, int color) {
        float half = diameter / 2f;
        drawRoundedBorder(matrices, centerX - half, centerY - half, diameter, diameter, circleRounding(diameter), thickness, color);
    }

    private static float circleRounding(float diameter) {
        return diameter * 0.35f;
    }
    ...
    public static void drawCircle(MatrixStack matrix, float centerX, float centerY, float diameter, int color) {
        float half = diameter / 2f;
        drawRoundedRect(matrix, centerX - half, centerY - half, diameter, diameter, circleRounding2(diameter), color);
    }
    ...
    public static class render3D {
        ...
        public static void setTranslation(MatrixStack matrixStack) {
            render3D.lastProjMat.set(RenderSystem.getProjectionMatrix());
        ...
        public static void endBuildingBoundProgram(BufferBuilder bb) {
            BuiltBuffer builtBuffer = bb.endNullable();
            if (builtBuffer != null) {
