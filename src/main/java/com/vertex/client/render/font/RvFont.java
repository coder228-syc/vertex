...
public class RvFont {
    ...
    public RvFont(InputStream fontStream, float size) {
        this.fontSize = size;
    ...
    private void generateAtlas() {
        BufferedImage img = new BufferedImage(2048, 2048, 2);
        ...
        for (int i = 0; i < charsToRender.length(); i++) {
            ...
            if (curX + iCharW + 8 >= 2048) {
                curX = 6;
                curY += maxRowHeight + 8;
                maxRowHeight = 0;
            }

            if (curY + iCharH + 8 >= 2048) {
                break;
            }
            ...
            glyph.u1 = curX / 2048.0F;
            glyph.v1 = curY / 2048.0F;
            glyph.u2 = (curX + charW) / 2048.0F;
            glyph.v2 = (curY + charH) / 2048.0F;
        ...
        try {
            NativeImage nativeImage = new NativeImage(2048, 2048, true);

            for (int y = 0; y < 2048; y++) {
                for (int x = 0; x < 2048; x++) {
                    int argb = img.getRGB(x, y);
                    int alpha = argb >> 16 & 0xFF;
                    nativeImage.setColorArgb(x, y, alpha << 24 | 16777215);
                }
            }
            ...
            this.textureId = Identifier.of("vertexclient", "fonts/" + safeFontName + "_" + (int) (this.fontSize * 10.0F));
            IMinecraft.getMc().getTextureManager().registerTexture(this.textureId, tex);
        ...
        if (text != null && !text.isEmpty() && this.textureId != null) {
            ...
            for (int i = 0; i < text.length(); i++) {
                ...
                if (c == SECTION && i + 1 < text.length()) {
                    ...
                    if (glyph != null) {
                        float x2 = curX + glyph.width;
                        float y2 = y + glyph.height;
                        bufferBuilder.vertex(matrix, curX, y2, 0.0F).texture(glyph.u1, glyph.v2).color(r, g, b, a);
                        bufferBuilder.vertex(matrix, x2, y2, 0.0F).texture(glyph.u2, glyph.v2).color(r, g, b, a);
                        bufferBuilder.vertex(matrix, x2, y, 0.0F).texture(glyph.u2, glyph.v1).color(r, g, b, a);
                        bufferBuilder.vertex(matrix, curX, y, 0.0F).texture(glyph.u1, glyph.v1).color(r, g, b, a);
                        curX += glyph.width;
                    }
                }
            }
