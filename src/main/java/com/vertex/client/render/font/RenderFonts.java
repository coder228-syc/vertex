...
public class RenderFonts implements Closeable, IMinecraft {
    private static final String OBFUSCATION_CHARS = "AĄBCĆDEĘFGHIJKLŁMNŃOÓPRSŚTUWYZŹŻaąbcćdeęfghijklłmnńoóprsśtuwyzźż0123456789";
    ...
    public void drawFadeLeftAligned(MatrixStack ms, String text, float x, float y, int color, float clipWidth) {
        renderFade(ms, text, x, y, color, clipWidth);
    }
    ...
    public float drawText(MatrixStack matrixStack, Text component, float x, float y) {
        return renderText(matrixStack, component, x, y, -1);
    }

    public float drawText(MatrixStack matrixStack, Text component, float x, float y, int color) {
    ...
    public float getTextWidth(Text text) {
        ...
        text.asOrderedText().accept((index, style, codePoint) -> {
            ...
            if (style.isObfuscated() && codePoint != 32) {
                advance = getWidth(new String(Character.toChars(codePoint)));
            } else {
                str = ReplaceUtil.replaceCustomFonts(new String(Character.toChars(codePoint)));
                advance = getWidth(str);
            }
            width[0] += advance;
            return true;
        });
    ...
    public float renderText(MatrixStack matrixStack, Text component, float x, float y, int defaultColor) {
        float startX = x;
        final float[] posX = {startX};
        component.asOrderedText().accept((index, style, codePoint) -> {
            ...
            float advance;
            if (style.isObfuscated() && codePoint != 32) {
                str = String.valueOf(getObfuscatedChar(index));
                advance = getWidth(new String(Character.toChars(codePoint)));
            } else {
                str = ReplaceUtil.replaceCustomFonts(new String(Character.toChars(codePoint)));
                advance = -1f;
            }
            float rendered = renderColoredString(matrixStack, str, posX[0], y, color);
            posX[0] += advance >= 0f ? advance : rendered;
            return true;
        });
        return posX[0] - startX;
    }

    private static char getObfuscatedChar(int index) {
        long seed = index * 31L + (System.currentTimeMillis() / 50L);
        int i = (int) (Math.abs(seed) % OBFUSCATION_CHARS.length());
        return OBFUSCATION_CHARS.charAt(i);
    }

    private float renderColoredString(MatrixStack ms, String text, float x, float y, int color) {
        if (text == null || text.isEmpty()) return 0f;
        renderRankText(ms, text, x, y, color);
    ...
    private void renderRankText(MatrixStack ms, String text, float x, float y, int color) {
        if (text == null || text.isEmpty()) return;
        ...
        char[] chars = getCharArray(text);
        int len = text.length();
        String upperText = RankTextGradient.upperCase(text);
        ...
        for (int i = 0; i < len; i++) {
            ...
            final float[] cursorRef = {cursorX};
            int gradientEnd = RankTextGradient.tryProcess(text, upperText, len, i, (ch, cr, cg, cb) -> {
                Glyph glyph = locateGlyph(ch);
                if (glyph != null) {
                    Identifier tex = glyph.owner().bindToTexture;
                    localCache.computeIfAbsent(tex, k -> new ObjectArrayList<>()).add(new DrawEntry(cursorRef[0], 0, cr, cg, cb, glyph));
                    cursorRef[0] += glyph.width();
                }
            });
            if (gradientEnd >= 0) {
                cursorX = cursorRef[0];
    ...
    private void renderFade(MatrixStack ms, String text, float x, float y, int color, float clipWidth) {
        if (text == null || text.isEmpty() || clipWidth <= 0f) return;
        ...
        float clipRight = x + clipWidth;
        float fadeWidth = Math.min(16f, Math.max(8f, clipWidth * 0.35f));
        float baseA = ((color >>> COLOR_SHIFT_ALPHA) & 0xFF) * INV_COLOR_DIVISOR;
        float r = ((color >>> COLOR_SHIFT_RED) & 0xFF) * INV_COLOR_DIVISOR;
        float g = ((color >>> COLOR_SHIFT_GREEN) & 0xFF) * INV_COLOR_DIVISOR;
        float b = (color & 0xFF) * INV_COLOR_DIVISOR;
        ...
        Scissor.push();
        try {
            Scissor.setFromComponentCoordinates(x, y - 1f, clipWidth, getHeight() + 2f);
            ...
            localCache.forEach((identifier, entries) -> {
                ...
                for (DrawEntry entry : entries) {
                    ...
                    float glyphLeft = x + entry.atX * SCALE_FACTOR;
                    float glyphRight = glyphLeft + w * SCALE_FACTOR;
                    float aLeft = fadeAlpha(glyphLeft, clipRight, fadeWidth, baseA);
                    float aRight = fadeAlpha(glyphRight, clipRight, fadeWidth, baseA);

                    bufferBuilder.vertex(matrix4f, entry.atX, entry.atY + h, 0).texture(u1, v2).color(entry.r, entry.g, entry.b, aLeft);
                    bufferBuilder.vertex(matrix4f, entry.atX + w, entry.atY + h, 0).texture(u2, v2).color(entry.r, entry.g, entry.b, aRight);
                    bufferBuilder.vertex(matrix4f, entry.atX + w, entry.atY, 0).texture(u2, v1).color(entry.r, entry.g, entry.b, aRight);
                    bufferBuilder.vertex(matrix4f, entry.atX, entry.atY, 0).texture(u1, v1).color(entry.r, entry.g, entry.b, aLeft);
                }
                RenderUtil.render3D.endBuilding(bufferBuilder);
            });
    ...
    private static float fadeAlpha(float screenX, float clipRight, float fadeWidth, float baseAlpha) {
        if (screenX <= clipRight - fadeWidth) return baseAlpha;
        if (screenX >= clipRight) return 0f;
        float t = MathHelper.clamp((clipRight - screenX) / fadeWidth, 0f, 1f);
        t = t * t * (3f - 2f * t);
        return baseAlpha * t;
    }
    ...
    private class GlyphMap {
        ...
        private void registerBufferedImageTexture(Identifier id, BufferedImage img) {
            try (NativeImage nativeImage = new NativeImage(NativeImage.Format.RGBA, img.getWidth(), img.getHeight(), false)) {
                ...
                // Без этого GL растягивает атлас шрифта с линейной фильтрацией -> текст выглядит размытым
